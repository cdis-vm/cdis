...
class BasicBlock:
    ...
    def instructions(self) -> tuple[Instruction, ...]:
        return tuple(self.bytecode.instructions[self.start_index_inclusive:self.end_index_exclusive])
...
def resolve_stack_metadata(bytecode: 'Bytecode') -> tuple[StackMetadata, ...]:
    ...
    for instruction in bytecode.instructions:
        ...
        next_instruction_was_skipped = (instruction.bytecode_index + 1 not in
                                        instruction.opcode.next_bytecode_indices(instruction))
    basic_blocks: list[BasicBlock] = []
    jump_target_to_basic_block: dict[int, BasicBlock] = {}

    for index in range(len(leader_indices) - 1):
        basic_block = BasicBlock(bytecode=bytecode,
    ...
    for basic_block in basic_blocks:
        for instruction in basic_block.instructions:
            current_stack_metadata = (
                ...
                if instruction.bytecode_index in opcode_index_to_stack_metadata else
                StackMetadata.dead_code())
            if current_stack_metadata.dead:
                opcode_index_to_stack_metadata[instruction.bytecode_index] = current_stack_metadata
    ...
    for exception_handler in bytecode.exception_handlers:
        ...
        exception_stack_metadata = replace(
            ...
            stack=(ValueSource(
                sources=(),
                value_type=exception_handler.exception_class),))
        if target_index in opcode_index_to_stack_metadata:
            opcode_index_to_stack_metadata[target_index] = (
                exception_stack_metadata.unify_with(opcode_index_to_stack_metadata[target_index]))
        else:
    ...
    while has_changed:  # Keep unifying until no changes are detected
        ...
        for basic_block in basic_blocks:
            for instruction in basic_block.instructions:
                original_instruction_metadata = opcode_index_to_stack_metadata[instruction.bytecode_index]
                ...
                next_stack_metadata_for_branches = (
                    ...
                    else instruction.opcode.next_stack_metadata(instruction, bytecode, original_instruction_metadata)
                )
                ...
                for branch_index in range(len(branches)):
                    ...
                    original_opcode_metadata = opcode_index_to_stack_metadata[branch_target]
                    try:
                        new_opcode_metadata = original_opcode_metadata.unify_with(next_stack_metadata)
                    except ValueError as e:
                        raise ValueError(f'Stack metadata mismatch for bytecode index {branch_target} for bytecode\n{bytecode}') from e
                    opcode_index_to_stack_metadata[branch_target] = new_opcode_metadata
                    has_changed |= (new_opcode_metadata != original_opcode_metadata)

        for exception_handler in bytecode.exception_handlers:
            target_index = exception_handler.handler_label.index
            original_exception_stack_metadata = opcode_index_to_stack_metadata[target_index]
            new_exception_stack_metadata = replace(
                opcode_index_to_stack_metadata[exception_handler.from_label.index],
                stack=(ValueSource(
                    sources=(),
                    value_type=exception_handler.exception_class),)).unify_with(original_exception_stack_metadata)
            opcode_index_to_stack_metadata[target_index] = new_exception_stack_metadata
            has_changed |= (new_exception_stack_metadata != original_exception_stack_metadata)

    return tuple(value[1] for value in sorted(opcode_index_to_stack_metadata.items(), key=lambda item: item[0]))
